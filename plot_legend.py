...
def create_barplot_legend_image():
    ...
    patches = [mpatches.Patch(color='none', label=f"{i}: {label}") for i, label in enumerate(labels, 1)]
    ...
    legend = ax.legend(
        ...
        columnspacing=1.5,  # Adjust column spacing
        handletextpad=2  # Adjust text spacing
    )
