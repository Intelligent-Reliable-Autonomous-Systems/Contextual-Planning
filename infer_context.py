...
def get_context_from_inference(domain, start_state='random', with_exp_traj=False):
    ...
    for te_instance_idx, te_file in enumerate(test_filenames):
        ...
        inferred_contexts_as_dict = dataframe_to_dict(inferred_contexts_bayes)
        return inferred_contexts_as_dict
    


def get_context_from_inference_for_grid(domain, filename, start_state='random', with_exp_traj=False):
